...
def run(G,heuristic,percentage):
    ...
    best_edges = edges_insert(list_ed,list_prob,int((G.ecount()*percentage)/100))
    G_m = deepcopy(G)
...
def edges_insert(list_ed,list_prob,len_insert):
    ...
    """
    i_prob = sorted(range(len(list_prob)), key=lambda k: list_prob[k],reverse = True)
    value = list_prob[i_prob[len_insert - 1]]
    L1 = [i for i in i_prob if value == list_prob[i]]
    L2 = [i for i in i_prob[:len_insert] if list_prob[i]!=value ]
    L3 = L2 + list(np.random.choice(L1,len_insert - len(L2),replace=False))
    return [list_ed[i] for i in L3]
